...
if (empty($dept) && empty($prod)) {
    $sql = "SELECT listId, line,jigName, jigLocation, lineStatus, technicianName,category, department, startDateTime,operatorName, department,backupComment,machineName,machineNo,process, DATE_FORMAT(requestDateTime, '%Y-%m-%d') as dateRequest, DATE_FORMAT(requestDateTime, '%h:%i %p') as timeRequest  FROM tblandonongoing";
    $query = $db->query($sql);
    // LOOP DATA
...
elseif (empty($prod) && !empty($dept)) {
    $sql = "SELECT listId, line,category,jigName, jigLocation, lineStatus, technicianName, department, startDateTime,operatorName,department,backupComment,machineName,machineNo,process, DATE_FORMAT(requestDateTime, '%Y-%m-%d') as dateRequest, DATE_FORMAT(requestDateTime, '%h:%i %p') as timeRequest  FROM tblandonongoing WHERE department ='$dept'";
    $query = $db->query($sql);
    while ($res = $query->fetch_assoc()) {
        if ($res['machineNo'] == 'N/A') {
            $res['machineNo'] = '';
...
elseif (!empty($prod) && empty($dept)) {
    ...
    $sql = "SELECT listId, line,category, jigName, jigLocation, lineStatus,technicianName, department, startDateTime,operatorName,department,backupComment,machineName,machineNo,process, DATE_FORMAT(requestDateTime, '%Y-%m-%d') as dateRequest, DATE_FORMAT(requestDateTime, '%h:%i %p') as timeRequest  FROM tblandonongoing WHERE department ='$dept' AND category = '$prod'";
    $query = $db->query($sql);
    while ($res = $query->fetch_assoc()) {
        if ($res['machineNo'] == 'N/A') {
            $res['machineNo'] = '';
